...
double FormulaParser::parseFormula(const string& formula, const vector<vector<Cell>>& sheet) {
...
float FormulaParser::parseAssignment(const string& formula, const vector<vector<Cell>>& sheet) {
    return parseExpression(formula, sheet);
}

double FormulaParser::parseExpression(const string& formula, const vector<vector<Cell>>& sheet) {
    istringstream ss(formula);
    string token;

    vector<double> values; // for store values
    vector<char> operators; // store operators

    // divide the formula
    while (ss >> token) {
        if (token == "+" || token == "-" || token == "*" || token == "/") {
            operators.push_back(token[0]); // save the operator
        } else {
            double value;

            if (isalpha(token[0]) && token.find('(') == string::npos) {
                value = getCellValue(token, sheet); // cell value
                ...
                value = stod(token); // convert to number
            }

            // first do * and /
            if (!operators.empty() && (operators.back() == '*' || operators.back() == '/')) {
                char op = operators.back(); // find end operator
                operators.pop_back(); // pop operator

                double lastValue = values.back(); //return the last element
                values.pop_back(); // poop this element
            ...
            // add new result in values for other operatr
...
float FormulaParser::getCellValue(const string& token, const vector<vector<Cell>>& sheet) {
    int row = token[1] - '1'; // for row
    int col = token[0] - 'A'; // for col -'A'
    return sheet[row][col].getValue();
}

float FormulaParser::parseFunction(const string& formula, const vector<vector<Cell>>& sheet, const string& function) {
    size_t start = formula.find("(");
    size_t end = formula.find(")");

    if (start == string::npos || end == string::npos) { //if function given wrong format
        throw invalid_argument("Invalid function format.");
    }

    string args = formula.substr(start + 1, end - start - 1); //formula without ()
    // search for a which function calls
    if (function == "MAX") {
...
double FormulaParser:: maxv(const string& range, const vector<vector<Cell>>& sheet) {
...
double FormulaParser::average(const string& range, const vector<vector<Cell>>& sheet) {
    char startCol = range[0];
    int startRow = stoi(range.substr(1, range.find("..") - 1)) - 1;
    char endCol = range[range.find("..") + 2];
...
double FormulaParser::sum(const string& range, const vector<vector<Cell>>& sheet) {
     char startCol = range[0];
    int startRow = stoi(range.substr(1, range.find("..") - 1)) - 1;
    char endCol = range[range.find("..") + 2];
    int endRow = stoi(range.substr(range.find("..") + 3)) - 1;
...
double FormulaParser::stddev(const string& range, const vector<vector<Cell>>& sheet) {
...
double FormulaParser::minv(const string& range, const vector<vector<Cell>>& sheet) {
    char startCol = range[0];
    int startRow = stoi(range.substr(1, range.find("..") - 1)) - 1;
    char endCol = range[range.find("..") + 2];
    int endRow = stoi(range.substr(range.find("..") + 3)) - 1;
